...
export class SummaryRepository {
  ...
  async getMoodSummary(start: Date, end: Date, userId: string) {
    ...
    >`
      SELECT 
        DATE("created_at" AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok') AS date,
        AVG("mood_score" + 3)::float AS "avgMood",
        COUNT(*)::int AS "count"
      FROM "entries"
      WHERE "is_delete" = false
        AND "user_id" = ${userId}
        AND "created_at" BETWEEN ${start} AND ${end}
      GROUP BY DATE("created_at" AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Bangkok')
      ORDER BY date ASC
    `;
    return raw;
  }
